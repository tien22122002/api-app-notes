...
import 'package:notes_app/bottom_bar/bottom_bar_item.dart';
...
class _BottomBarBubbleState extends State<BottomBarBubble>
      ...
      decoration: BoxDecoration(
        boxShadow: [
          BoxShadow(
            ...
            blurRadius: 5, // Bán kính của mờ
            offset: const Offset(0, 3), // Độ dịch chuyển của đổ bóng
          ),
        ],
      ),
      child: Container(
  ...
  Future _onChangeIndex(int index) async {
    if (_isChangingIcon) {
      return; // Nếu đang thay đổi biểu tượng, không thực hiện thêm thay đổi
    }
    ...
    for (int i = 0; i < iconsKey.length; i++) {
      iconsKey[i].currentState?.updateSelect(i == index);
    }
    setState(() {
